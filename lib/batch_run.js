...
export async function batch_run(ns, target, scripts, counts, delays, batchTag=0) {
    ...
    // ns.print(target, scripts, counts, delays);
    var start = Date.now();
    while(worker) {
        if (running >= counts[iterator]) {
            ns.print(`running all ${counts[iterator]} batches of ${scripts[iterator]}, moving to next`);
            iterator += 1;
            running = 0;
        }
        ...
        if (remainingBatches == 0){
            // ns.print(`now running all batches of ${scripts[iterator]}, breaking`);
            iterator += 1;
            continue;
        }
